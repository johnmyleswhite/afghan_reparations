...
jags <- jags.model(file.path('jags', 'province_hierarchical_gamma.bug'),
                   data = list('payment' = claims$AgreedPayment,
                               'province' = as.numeric(claims$Province),
                               'N' = nrow(claims),
                               'J' = max(as.numeric(claims$Province))),
                   n.chains = 4,
                   n.adapt = 1000)
...
mcmc.save(mcmc.samples,
          filename = file.path('graphs', 'province_hierarchical_gamma'))
...
ggsave(filename = file.path('graphs', 'province_parameters_1.png'), plot = p)
...
p <- ggplot(credible.intervals[5:8,],
  ...
  xlab('') +
  scale_color_discrete(legend = FALSE)
ggsave(filename = file.path('graphs', 'province_parameters_2.png'), plot = p)
